...
`JamesHWade/tempest`. Read [the issue workflow](dev/agents/issue-tracker.md)
when finding, creating, updating, or closing work.

## This package

`tempest` is an R package that ports STORM and Co-STORM research workflows to
R. The package has three main surfaces:

- Scripted STORM runs in `R/storm*.R`, with structured steps routed through
  `dsprrr` modules from `R/dsprrr.R`.
- Interactive Co-STORM sessions in `R/costorm*.R`, including expert tools,
  mind-map state, and session report generation.
- The bundled Shiny app in `inst/shiny/`, which wraps those package APIs rather
  than carrying separate research logic.
...
- Tests must not require API keys, network access, or live provider responses.
  Use fake chats, local stores, package fixtures, and focused Shiny `testServer`
  checks for deterministic coverage.
- For Shiny hangs or async regressions, reproduce the live app path when
  possible. Pay attention to session lifetime, background `ExtendedTask` work,
  reactive invalidation, and synchronous expert tool calls.
- When changing public APIs, update roxygen, generated `man/` files,
  `_pkgdown.yml` for new topics, `NEWS.md`, and README examples as needed.
- For focused local checks, prefer `devtools::test(filter = "...")` or load the
  package namespace before direct `testthat::test_file()` runs so tests exercise
  the current checkout instead of an installed stale package.
