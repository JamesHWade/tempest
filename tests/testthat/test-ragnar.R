test_that("storm_create_ragnar_store creates store with correct schema", {
...
test_that("StormConfig creates ragnar_store from embed_fn", {
  skip_if_not_installed("ragnar")
...
test_that("StormConfig accepts pre-built ragnar_store", {
  skip_if_not_installed("ragnar")

  mock_embed <- function(x) matrix(stats::rnorm(length(x) * 3), ncol = 3)
  pre_store <- storm_create_ragnar_store(mock_embed, cache_dir = NULL)
  cfg <- storm_config(ragnar_store = pre_store)

  expect_identical(cfg$ragnar_store, pre_store)
})

test_that("StormRetriever inherits ragnar_store from config", {
  skip_if_not_installed("ragnar")
