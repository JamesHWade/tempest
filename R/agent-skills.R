...
tempest_agent_skills <- function() {
  ...
  paths <- paths[file.exists(file.path(paths, "SKILL.md"))]
  paths <- sort(paths)
  if (length(paths) == 0L) {
    tempest_agent_skill_abort(
      "The installed {.pkg tempest} package contains no valid Agent Skills."
    )
  }
...
#' @param skills Bundled skill names to install. `NULL` installs all bundled
#'   skills.
