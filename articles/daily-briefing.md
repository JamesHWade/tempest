...
The first day has nothing to carry forward, so it searches. Every later
day starts from the records the previous run carried and adds what Graft
reports as accepted since the previous snapshot, scoped to this
briefing: a changed `Claim` joins the basis when it matches the
monitored topic, while a lifecycle change to a claim already in the
basis is retained so a retraction or supersession can remove it. A
changed `ClaimSupport` joins only when it supports a claim in the basis.
The basis therefore grows only through explicit acceptance of relevant
knowledge; a keyword search that happens to match a different set of
records on day 40 cannot add unrelated claims, and a shared
organizational store cannot flood it with other topics. Evidence spans
and sources are carried as accepted because this store is dedicated to
the briefing; filter them the same way if you share a store.

[`tempest_knowledge()`](https://jameshwade.github.io/tempest/reference/tempest_knowledge.md)
reads at most 1,000 records, so the basis is a bounded table rather than
an ever-growing id list. Every active `Claim` is always kept because the
claims are the change signal; evidence records fill the remaining room
newest-first, and a record whose latest revision is retracted or
superseded, or whose change is a deletion, drops out. If a monitored
topic ever accumulates more than 1,000 active claims, the run stops
rather than forgetting some of them: narrow the topic or give it its own
store.
...
in_scope <- function(changed, basis, scope_view = view) {
  basis_claims <- basis$record_id[basis$class == "Claim"]
  basis_record_ids <- basis$record_id
  matches <- graft_find(
    scope_view,
    topic_query,
    class = "Claim",
  ...
  deleted_from_basis <-
    changed$action == "delete" & changed$record_id %in% basis_record_ids
  keep <- ifelse(
    changed$class == "Claim",
...
changes_as_basis <- function(changed) {
  data.frame(
    ...
    status = vapply(
      ...
      function(index) {
        if (identical(changed$action[[index]], "delete")) {
          return("deleted")
        }
        record <- changed$record[[index]]
        if (is.null(record$status)) "active" else record$status
      },
      character(1)
    )
  )
...
if (file.exists(basis_path)) {
  ...
  changed <- in_scope(changed, previous$records)
...
compares two committed boundaries store-wide and returns one row per
changed record with its action, the union of changed public fields, and
the latest accepted record, whose `status` tells the basis whether the
record is still active. A `GraftView` bounds it at the pinned snapshot,
so the selection cannot include a commit that landed after the run
started.
...
The saved snapshot always matches the saved records. Without a commit it
is the snapshot this run pinned, so anything accepted by another process
since then is picked up tomorrow. After a commit it is the post-commit
snapshot, and the basis absorbs every record accepted between the pin
and that boundary, including a concurrent commit by another process.
Rejection leaves accepted knowledge unchanged. This two-speed loop keeps
routine monitoring passive and makes only material changes enter the
slower decision-and-review path.
