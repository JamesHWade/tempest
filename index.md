...
key required (fallback when native not available) - Serper: set
`SERPER_API_KEY` - Brave: set `BRAVE_API_KEY` - Tavily: set
`TAVILY_API_KEY`
...
[`tempest_run()`](reference/tempest_run.md) executes five steps:
`perspectives`, `research`, `outline`, `write`, `polish`. Key features:
...
[`tempest_config()`](reference/tempest_config.md) accepts the following
parameters:
...
- **Co-STORM**: Jiang, Y., et al. (2024). [Into the Unknown Unknowns:
  ...
  Conversations](https://arxiv.org/abs/2408.15232). arXiv preprint.
