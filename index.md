...
reads the Graft producer. New research remains provisional until the
host explicitly reviews and accepts it.
...
Runtime loading accepts contracts `>= 0.2.0` and `< 3.0.0` with store
format `3.1.0`, covering the tested 0.x, 1.x and 2.x major lines. The
1.0 data-dict manifest cutoff does not affect Tempest’s pinned LinkML
...
instead of packaging unselected evidence. Planning is read-only: Tempest
does not call
[`graft::graft_commit()`](https://jameshwade.github.io/graft/reference/graft_commit.html)
on the host’s behalf. The promotion directory is a closed current-format
bundle, its destination must not already exist, and any older or extra
shape is rejected. Reading requires the original bundle id as an
out-of-band trust pin; checksums stored inside the directory establish
internal consistency but are not a signature. Research promotion never
creates executable authority; stored program references are provenance
only.
...
snapshot, bundle, publication, and recorded decision identities
establish explicit bindings; `correlation_id` can establish only
`correlated_with` with `correlation_only` proof. It never establishes
authorship or causation. Mutable progress events are intentionally
outside the review identity. A promotion bundle or publication is shown
as proposed; an exact recorded accept decision is shown as historically
accepted. Withdrawal does not erase that history or alter the original
input selection. An input `reported_decision` is unverified host
provenance, not authenticated acceptance; malformed metadata is omitted
from the projection. Current reuse still requires fresh admission
through
[`tempest_reuse_artifact_research()`](https://jameshwade.github.io/tempest/reference/tempest_reuse_artifact_research.md).
Schema 2 directly replaces the old receipt-based review; cross-product
publications are rejected.

The current persistence line accepts only `ResearchWorkspace` snapshot
schema 6, Co-STORM snapshot and bundle schema 12, STORM bundle schema 9
with state schema 5, ProgramSet schema 2, research-manifest schema 4,
...
workspace, product state, report, and optional immutable Graft snapshot.
Fixed scientific transformations carry dsprrr program identity, while
...
Each run directory is an exact current schema-9 STORM product bundle
...
Accepted evidence uses the same immutable knowledge boundary. Preserve
complete host-selected promotion receipts so each claim keeps its
supports, evidence spans and sources. The shared host recipe retains
...
current schema-12 product. Expert, transcript, mind-map, StageRecord,
Workspace, report, suggested-question, and Graft snapshot state must
