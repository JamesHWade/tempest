...
Every evidence value written to a workspace is an exact
`TempestResource` created by
[`tempest_resource()`](https://jameshwade.github.io/tempest/reference/tempest_resource.md).
Retriever fetches and provider-native tools produce that same record
directly, and cached or restored evidence must have the identical
current shape. Source-shaped values returned by source-inspection
methods are read-only presentation views; they are not accepted as
mutable workspace inputs, and Tempest provides no legacy list conversion
path.
...
schema 5, Co-STORM snapshot and bundle schema 10, STORM bundle schema 8
...
session; it does not generate or repair a report. `tempest_report_md()`
...
current schema-10 product. Expert, transcript, mind-map, StageRecord,
